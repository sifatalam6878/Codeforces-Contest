...
#include<bits/stdc++.h>
using namespace std;
long long ddd(int kkk,vector<long long>xx)
{
    if(kkk==0)
    //subhanallah
      return 3;
      //masallah
     return xx[kkk+1]+1LL*kkk*xx[kkk-1];
}
...
int main() {
    ...
    while(siu--)
    {
        ...
        long long m=min(d,lll);
        m -=((m-s)&1LL);
        //subhanallah
        long long sff = s;
        for(int j=20;j>=1&&sff<m;j--)
        {
            if(fkk[j]==0)
               continue;
            long long knk=(m-sff)/2;
            //masallah
            long long ttt=min(fkk[j],knk);
            if(ttt==0)
              continue;
            fkk[j]-=ttt;
            //masllah
